...
public class SearchProjectsQueryBuilderValidator {
  ...
  public void validate(SearchProjectsCriteriaQuery query) {
    ...
    try (DbSession dbSession = dbClient.openSession(false)) {
      ...
      if (metricDtos.size() != metricKeys.size()) {
        ...
        List<String> unknownKeys = metricKeys.stream().filter(metricKey -> !metricDtoKeys.contains(metricKey)).collect(Collectors.toList());
        throw new IllegalArgumentException(String.format("Unknown metric keys '%'", unknownKeys));
      }
    }
  }
}
